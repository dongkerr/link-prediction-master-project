...
#calculate the weight of edge based on the date
def getWeight(dateString):
  # Getting the current date and time
  dt = datetime.now()
  print(dt)
  # getting current timestamp
  tsNow = datetime.timestamp(dt)
  print(tsNow)
  #getting input date timestamp
  tsInput=time.mktime(datetime.strptime(dateString, "%Y-%m-%d").timetuple())

  weight = tsNow/(tsNow-tsInput)
  return weight

def buildEdge(day,yearLen,year0,month,graphList):
  xList = []
  yList = []
  dictList = {}
  weightList = []
  for i in range(yearLen):
    #date = year+'-'+month+'-'+str(day0+i)
    date = str(year0 + i) + '-' + month + '-' + day
    graph = graphList[i]
...
month = '12'
graphList = []
path = "/home/user/weizi/data/"

#reading data to build graph
for i in range(yearLen):
  date = str(year0+i)+'-'+month+'-'+day
  file = path+date+'.graphml'
  graph = nx.read_graphml(file)
  graphList.append(graph)
edgesList = buildEdge(day,yearLen,year0,month,graphList)

print('edgesList,len(edgesList):',len(edgesList),edgesList)
...
stellarGraph_features = buildStellarGraph(edgesList)
print(stellarGraph_features.info())
...
num_edges_graph = int(len(edgesList) * (1 - train_subset))

# keep older edges in graph, and predict more recent edges
edges_graph = edgesList[:num_edges_graph]
edges_other = edgesList[num_edges_graph:]
...
graph = stellarGraph_features = StellarDiGraph(
    edges=edges_graph,
    node_type_default="user",
     edge_type_default="send",
     edge_weight_column="weight"
...
#for test
fileTest="/home/user/weizi/data/2020-12-01.graphml"
graphTest = nx.read_graphml(fileTest)
dayTest = '01'
monthLenTest = 1
yearTest = 2020
monthTest = '12'
edgesTest = buildEdge(dayTest,monthLenTest,yearTest,monthTest,[graphTest])
graphTest = buildStellarGraph(edgesTest)
print(graphTest.info())

posTest, negTest = positive_and_negative_links(graphTest, edgesTest)
link_examples_Test, link_labels_Test = labelled_links(posTest, negTest)
